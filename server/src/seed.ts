...
// Generate 10 random pipelines
const languages = ['node', 'python', 'java', 'yaml'];
const repos = ['github.com/org/service-alpha', 'github.com/org/service-beta', 'github.com/org/service-gamma', 'github.com/org/data-processor', 'github.com/org/api-gateway'];
const commandPool = [
  'npm install && npm test',
  'pip install -r requirements.txt && pytest',
  'mvn clean install',
  'docker build -t app:latest .',
  'if [ $((RANDOM % 2)) -eq 0 ]; then echo "Randomly failed"; exit 1; else echo "Success"; exit 0; fi',
  'if [ $((RANDOM % 2)) -eq 0 ]; then echo "ECONNREFUSED: Connection refused by 127.0.0.1"; exit 1; else echo "Success"; exit 0; fi'
];

for (let i = 1; i <= 10; i++) {
  samplePipelines.push({
    name: `Auto-Generated Pipeline ${i}`,
    repository: repos[Math.floor(Math.random() * repos.length)],
    language: languages[Math.floor(Math.random() * languages.length)],
    steps: [
      { name: 'Setup', command: 'echo "Setting up environment"' },
      { name: 'Execute', command: commandPool[Math.floor(Math.random() * commandPool.length)] }
    ],
    envVars: []
  });
}

mongoose.connect(process.env.MONGODB_URI || 'mongodb://admin:password@localhost:27017/devops?authSource=admin')
  .then(async () => {
    console.log('Connected to MongoDB');
    await Pipeline.deleteMany({});
    await Pipeline.insertMany(samplePipelines);
