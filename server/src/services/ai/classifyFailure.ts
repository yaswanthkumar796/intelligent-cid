import { parseExecutionLogs } from './analyzeLogs';
import axios from 'axios';

export const classifyRootCause = async (logs: string) => {
  const parsedLogs = parseExecutionLogs(logs).toLowerCase() + ' ' + logs.toLowerCase();
  ...
  try {
    ...
    const response = await axios.post(`${mlEngineUrl}/predict`, {
      log_text: logs
    }, { timeout: 3000 });
